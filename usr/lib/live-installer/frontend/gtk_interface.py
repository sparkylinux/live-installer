...
class InstallerWindow:
    ...
    def __init__(self, fullscreen=False):
        ...
        self.wizard_pages[self.PAGE_INSTALL] = WizardPage(_("Installing SparkyLinux..."), "install.png")
    ...
    def build_lang_list(self):
        ...
        countries = {}
        for line in commands.getoutput("isoquery --iso 3166-1 | cut -f1,4-").split('\n'):
    ...
    def assign_language(self, treeview, data=None):
        ...
        if selection.count_selected_rows > 0:
            ...
            if iter is not None:
                ...
                self.setup.print_setup()
                gettext.translation('live-installer', "/usr/share/sparky/locale",
        ...
        if(not goback):
            if(sel == self.PAGE_LANGUAGE):
                ...
                if not found_root_partition:
                    ErrorDialog(_("Installation Tool"), "<b>%s</b>" % _("Please select a root (/) partition."), _("A root partition is needed to install SparkyLinux on.\n\n - Mount point: /\n - Recommended size: 30GB\n - Recommended filesystem format: ext4\n "))
                    return
