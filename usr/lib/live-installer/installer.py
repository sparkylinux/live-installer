...
gettext.install("live-installer", "/usr/share/sparky/locale")
...
class InstallerEngine:
    ...
    def __init__(self):
        ...
        else: name, version = 'SparkyLinux', '4'
    ...
    def init_install(self, setup):        
        ...
        if(not os.path.exists(self.media)):
            ...
            self.error_message(message=_("ERROR: Something is wrong with the installation medium! This is usually caused by burning tools which are not compatible with Sparky (YUMI or other multiboot tools). Please burn the ISO image to DVD/USB using a different tool."))
            return
        ...
        if (setup.gptonefi):
            os.system("mkdir -p /target/boot/efi/EFI/sparky")
            #os.system("cp /lib/live/mount/medium/EFI/BOOT/grubx64.efi /target/boot/efi/EFI/sparky")
            ...
            os.system("cp /lib/live/mount/medium/offline/* /target/debs/")
        ...
        for group in 'adm audio bluetooth cdrom dialout dip fax floppy fuse lpadmin netdev plugdev powerdev sambashare scanner sudo tape users video'.split():
        ...
        fp.close()
        self.do_run_in_chroot("pwconv && cat /tmp/.passwd | chpasswd")
        ...
        if setup.autologin:
            ...
            # SDDM
            self.do_run_in_chroot(r"sed -i -r -e 's/^#?(User)\s*=.*/\1={user}/' -e 's/^#?(Session)\s*=.*/\1=plasma.desktop/' /etc/sddm.conf".format(user=setup.username))
            # GDM3
    ...
    def do_check_grub(self, our_total, our_current):
        ...
        if os.path.exists("/target/boot/grub/grub.cfg"):
            ...
            for line in grubfh:
                line = line.rstrip("\r\n")
                if("06_sparky_theme" in line):
                    found_theme = True
                    print " --> Found Grub theme: %s " % line
                if ("menuentry" in line and "Sparky" in line):
                    found_entry = True
