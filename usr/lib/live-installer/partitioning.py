        ...
        try:
            ...
            if path_exists(mount_point, 'etc/'):
                description = getoutput("su -c '{{ . {0}/etc/lsb-release && echo $DISTRIB_DESCRIPTION; }} || \
                                                {{ . {0}/etc/os-release && echo $PRETTY_NAME; }}' sparky".format(mount_point)) or 'Unix'
            ...
            if path_exists(mount_point, 'etc/sparky/info'):
                description = getoutput("cat %s/etc/sparky/info | grep GRUB_TITLE" % mount_point).replace('GRUB_TITLE', '').replace('=', '').replace('"', '').strip()
